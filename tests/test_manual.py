...
from tests.test_setup import TestSetup, page_fixture
from utils.helpers import parametrize_generator

CONFIG_FILE = "test_config.json"
class TestHomePage(TestSetup):
    @pytest.mark.ui 
    @pytest.mark.parametrize(*parametrize_generator(CONFIG_FILE, "page_key", "expected_title"))
    def test_homepage_title(self, page_fixture, page_key, expected_title):
        """Test that the homepage title matches the expected title."""
        url = page_fixture[page_key]
        self.driver.get(url)
        assert self.driver.title == expected_title, f"Expected title '{expected_title}' but got '{self.driver.title}'"

    @pytest.mark.login
    @pytest.mark.parametrize(*parametrize_generator(CONFIG_FILE, "page_key", "expected_login_redirect"))
    def test_redirect_to_login(self, page_fixture, page_key, expected_login_redirect):
        url = page_fixture[page_key]
        self.driver.get(url)
        ...
        assert (
            is_redirected == expected_login_redirect ), f"Redirection check failed for {url}, expected: {expected_login_redirect}, got: {is_redirected}"
    ...
    class TestElementLoginPage(TestSetup):
        ...
        def test_login_text_present(self, pages_fixture, page_key, expected_login):
            ...
            assert is_present == expected_login, f"Login text presence check failed for {url}"
        @pytest.mark.login
        @pytest.mark.parametrize(
        ...
        def test_login_functionality(
            ...
            try:
                ...
                assert (
                    False
                ), "One of the elements (username, password, submit) was not found."
